...
class APIService {
    ...
    // DISABILITATA: Permettiamo localhost/127.0.0.1 per sviluppo locale
    private static func aggressiveCleanup() {
        // Non rimuovere più localhost/127.0.0.1 - necessario per sviluppo locale
    }
    ...
    private init() {
        ...
        if let savedURL = initialValue, (savedURL.contains("localhost") || savedURL.contains("127.0.0.1")) {
            ...
            print("✅ [APIService.init] UserDefaults pulito. Verrà usato il default: 'https://whatisbackend-production.up.railway.app'")
        }
    ...
    var baseURL: String {
        ...
        // URL pubblico di default (Railway cloud)
        // ✅ Backend deployato su Railway: sempre disponibile anche se il Mac è spento
        let defaultURL = "https://whatisbackend-production.up.railway.app"
        ...
        if let savedURL = rawUserDefaultsValue, !savedURL.isEmpty {
            ...
            // Usa direttamente l'URL salvato (anche se è localhost)
            print("✅ [APIService.baseURL] URL finale valido: '\(cleanedURL)'")
            print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            return cleanedURL
        }
        
        // Default: usa localhost per sviluppo
        print("🔍 [APIService.baseURL] Nessun URL salvato, uso default: '\(defaultURL)'")
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        return defaultURL
    }
    ...
    func fetchZone(zoneId: String) async throws -> Zone {
        ...
        request.httpMethod = "GET"
        request.cachePolicy = .reloadIgnoringLocalCacheData // ✅ Evita cache stale per ottenere sempre i dati più recenti
    ...
    func fetchAllZones() async throws -> [Zone] {
        // ✅ USA ENDPOINT MOBILE DEDICATO
        let urlString = "\(baseURL)/mobile/zones"
        print("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        ...
        var request = URLRequest(url: url)
        request.timeoutInterval = 30 // Timeout più ragionevole (30 secondi)
        request.httpMethod = "GET"
        request.cachePolicy = .reloadIgnoringLocalCacheData // ✅ Evita cache stale
        request.setValue("application/json", forHTTPHeaderField: "Accept")
