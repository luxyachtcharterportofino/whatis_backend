...
if [ ! -f "WhatisExplorer.xcodeproj/project.pbxproj" ]; then
    echo -e "${RED}❌ Errore: Esegui questo script dalla directory WhatisExplorer${NC}"
    exit 1
fi
...
# Verifica Bundle ID
BUNDLE_ID=$(grep -A 1 "PRODUCT_BUNDLE_IDENTIFIER" WhatisExplorer.xcodeproj/project.pbxproj | grep -o 'com\.andaly\.WhatisExplorer' | head -n 1)
...
# Verifica versione
VERSION=$(grep -A 1 "CFBundleShortVersionString" WhatisExplorer/Info.plist | grep -o '[0-9]\+\.[0-9]\+' | head -n 1)
BUILD=$(grep -A 1 "CFBundleVersion" WhatisExplorer/Info.plist | grep -o '[0-9]\+' | head -n 1)
...
echo ""
echo "1. ✅ Bundle ID: com.andaly.WhatisExplorer"
...
echo ""
echo "1. Apri Xcode:"
echo "   open WhatisExplorer.xcodeproj"
echo ""
